...
func Dashboard(wnd core.Window, model DashboardModel) core.View {
	...
	return ui.VStack(
		ui.H1("Übersicht Benutzerverwaltung"),
		cardlayout.Layout(
			...
			ui.If(model.Roles != "" && wnd.Subject().HasPermission(iam.ReadRole),
			...
			ui.If(model.Groups != "" && wnd.Subject().HasPermission(iam.ReadGroup),
			...
			ui.If(model.Permissions != "" && wnd.Subject().HasPermission(iam.ReadPermission),
