		...
		cfg.SetDecorator(func(wnd core.Window, view core.View) core.View {
			return scaffold(wnd, modAI.Assistant.Decorate(wnd, view, cfgai.AssistantOptions{
				...
				Agents: []uicompletion.Agent{{
					...
					// The screen tool is the only one built per window, because it looks at this window. It is
					// the feedback channel of the assistant: after lending a book, the model can check what the
					// user actually sees instead of assuming it worked. slices.Concat copies, so the shared
					// slice is never appended to from several windows at once.
