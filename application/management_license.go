...
func (c *Configurator) LicenseManagement() (LicenseManagement, error) {
	if c.licenseManagement == nil {
		...
		c.AddContextValue(core.ContextValue("nago.licenses.user", form.AnyUseCaseList[license.UserLicense, license.ID](func(subject auth.Subject) iter.Seq2[license.UserLicense, error] {
			return c.licenseManagement.UseCases.PerUser.FindAll(subject)
		})))

		c.AddContextValue(core.ContextValue("nago.licenses.app", form.AnyUseCaseList[license.AppLicense, license.ID](func(subject auth.Subject) iter.Seq2[license.AppLicense, error] {
			return c.licenseManagement.UseCases.PerApp.FindAll(subject)
		})))

		c.AddContextValue(core.ContextValue("nago.licenses.user.find_by_id", c.licenseManagement.UseCases.PerUser.FindByID))
	}
