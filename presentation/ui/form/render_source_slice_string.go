...
func RenderSourceSliceString(ctx FieldContext) core.View {
	field := ctx.Field()
	if !ctx.IsSlice(reflect.String) {
		return nil
	}
	...
	return picker.Picker[Entity](ctx.Label(), values, strState).
		Title(ctx.Label()).
		MultiSelect(true). // single select is another renderer, which only works on non-slice types
