...
| `Adaptive(light image.ID, dark image.ID)` | Adaptive sets the image to use different sources for light and dark themes. It generates URIs for both light and dark variants with fixed dimensions (512x512) and no fitting, then applies them to the image. |
...
| `EmbedAdaptive(light []byte, dark []byte)` | EmbedAdaptive is like [TImage.Embed] but picks whatever fits best. |
